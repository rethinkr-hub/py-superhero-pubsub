...
def sub_routine(msg):
    ...
    if isinstance(msg.body, bytes):
        content = json.loads(msg.body.decode('utf8'))
    elif isinstance(msg.body, str):
        content = json.loads(msg.body)
    
    if content and content['log_message'] == 'Cleaning Game Records':
        host = R_CONN.get('games:%s:host' % content['task']['game_token'])
        if host and host.decode('utf8') == content['task']['user_token']:
            etl_game_meta(content['task'])
            etl_game_podium(content['task'])
            
        if CLEAN_ROUTINE:
            R_CONN.expire('games:%s:superheros:%s' % (content['task']['game_token'], content['task']['user_token']), REDIS_EXPIRY)
